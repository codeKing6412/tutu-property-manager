...
(async()=>{
 ...
 await page.goto(base+'/#dashboard');await page.locator('h1').waitFor();assert.match(await page.locator('main').innerText(),/应收核算/);assert.match(await page.locator('main').innerText(),/所选范围未收账单/);assert.ok(await page.locator('#auditScope option').filter({hasText:'整栋：'}).count()>0);assert.ok(await page.locator('#auditScope option').filter({hasText:'整单元：'}).count()>0);const trendBars=page.locator('.bar[data-month]'),dashboardState=await (await page.request.get(base+'/api/state')).json();assert.equal(await trendBars.count(),6);assert.equal(await trendBars.last().getAttribute('data-month'),dashboardState.today.slice(0,7));await trendBars.last().hover();assert.equal(await page.locator('#chartTooltip').isVisible(),true);assert.match(await page.locator('#chartTooltip').innerText(),/实际进账.*笔收款/s);
 ...
 await page.goto(base+'/#settings');assert.match(await page.locator('main').innerText(),/住户身份选项/);assert.match(await page.locator('main').innerText(),/停车费到期提前提醒/);assert.match(await page.locator('main').innerText(),/批量导入与导出/);assert.match(await page.locator('main').innerText(),/一套房一行/);assert.equal(await page.locator('main [data-act="import"]').count(),4);assert.equal(await page.locator('main [data-act="export"]').count(),4);assert.equal(await page.locator('main [data-act="history-import"]').count(),1);await page.locator('[data-act="history-import"]').click();assert.match(await page.locator('#modal').innerText(),/不需要房屋编号、金额、收款日期或收款方式/);await page.locator('[data-act="close-modal"]').first().click();
