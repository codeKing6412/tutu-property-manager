...
(async()=>{
 ...
 await page.goto(base+'/#settings');assert.match(await page.locator('main').innerText(),/住户身份选项/);assert.match(await page.locator('main').innerText(),/停车费到期提前提醒/);
 assert.deepEqual(errors,[]);await browser.close();console.log('PASS: custom status in space management, split parking bundle, building scope, vehicle rule, flexible receivables audit, configurable labels and reminders.');
